...
- **Shared helpers** (used by multiple commands):
  ...
  - `_load_comfy_env` / `_comfy_discover` / `_comfy_preflight` / `_comfy_run` —
    ...
    sparkyard backend.
  - `_spark_guard_firewall` — hard-refuses to inject the gateway ADMIN key
    into a project whose `sandbox.yaml` weakens egress control (`firewall`
    not strict, any `allowed_domains`) **or** carries `features: [comfyui]`
