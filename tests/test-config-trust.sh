...
#
# Every fixture that actually reached `sandbox run` left
# sandbox-<name>-{claude,home,cache} volumes behind, and nothing else removes
# them — so each run of this suite littered the developer's machine a little
# more (the argtest-*, mnt-* and cfgtrust-* fixtures have done so since they
# were written). The project names are read back out of the fixture configs
# rather than kept as a literal list: several fixtures are generated by helper
# functions such as mk_args_proj, so a hand-kept list would silently drift and
# quietly stop cleaning up. Called from the trap BEFORE `rm -rf "$TMP"`, which
# deletes the configs it reads.
rm_fixture_volumes() {
    local cfg proj
    while IFS= read -r cfg; do
        [ -n "$cfg" ] || continue
        proj=$(yq -r '.name // ""' "$cfg" 2>/dev/null) || proj=""
        # Some fixtures END the suite as invalid YAML by design — the escape
        # hatch test appends a bare "x" to prove the write actually landed on
        # the host file — and yq then yields nothing, which used to let those
        # fixtures' volumes leak. The `name:` line itself is still intact, so
        # read it directly as a fallback.
        case "$proj" in
            ""|null) proj=$(sed -n 's/^name:[[:space:]]*//p' "$cfg" 2>/dev/null | head -1) ;;
        esac
        case "$proj" in ""|null) continue ;; esac
        docker volume rm -f "sandbox-${proj}-claude" "sandbox-${proj}-home" \
            "sandbox-${proj}-cache" >/dev/null 2>&1 || true
    done < <(find "$TMP" -name 'sandbox.yaml' -o -name 'sandbox.yml' 2>/dev/null)
}
